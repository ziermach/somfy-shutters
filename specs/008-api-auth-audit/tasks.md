...
- [X] T004 [P] Unit-test tokens in `backend/tests/unit/test_tokens.py`: prefix and length, alphabet, no `I L O U`, normalisation of `k7q-9xm` and ` K7Q9XM `, rejection of 5/7 characters and foreign letters, digest stability, 1 000 tokens all distinct
...
- [X] T009 [P] Unit-test the record in `backend/tests/unit/test_audit_store.py`: order by id survives wall times going backwards, filters, paging with `before`, purge, **`record` on a closed connection logs and returns** instead of raising, an entry keeps its `actor_name` after the credential is renamed or revoked
- [X] T010 Wire `AuthStore` and `AuditLog` in `backend/src/somfy_shutters/main.py` on `store.path`, close them on shutdown, expose as `app.state.auth` and `app.state.audit`
